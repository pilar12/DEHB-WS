...
if __name__ == "__main__":
    ...
    diverged, metrics, subnet, endtime = train_subnet(
        supernet, config_3x3, seed=101, device=device, budget=5, validate=True, test=True
    )
    ...
    diverged, metrics, subnet, endtime2 = train_subnet(
        subnet, config_1x1, seed=101, device=device, budget=5, validate=True, test=True
    )
    runtime2 = endtime2 - start_time2
    print(subnet is supernet)
    print(metrics)
    print(runtime2 + runtime)
    torch.save(subnet.state_dict(), "model/warmSupernet.state")
