...
def construct_model_optimizer(model: nn.Module, cfg: Configuration) -> Tuple[
    Optimizer, CosineAnnealingLR, nn.CrossEntropyLoss]:
...
def train_subnet(model: nn.Module, config: Configuration, seed: int, device: int, budget: int, validate: bool = True,
                 test: bool = True, train: bool = True) -> Tuple[bool, Dict, nn.Module, int]:
